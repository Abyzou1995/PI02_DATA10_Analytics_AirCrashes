...
if st.session_state['btn']:
    ...
    with a2:
        ...
        All_fat1["All Aboard"]
        All_fat1["Rate goal"] = 100*All_fat1["All Fatalities"]/All_fat1["All Aboard"]
        for i in range(All_fat1.shape[0]-1):
            All_fat1["Rate goal"].iloc[i+1] = All_fat1["Rate"].iloc[i]-5
        for i in range(All_fat1.shape[0]-1):
            All_fat1["Rate goal"].iloc[i+1] = All_fat1["Rate"].iloc[i]-5
        st.dataframe(All_fat1[(All_fat1["Year"] >= values[0]) &  (All_fat1["Year"] <= values[1])])
