...
st.markdown("<h1 style='text-align: center; color: black;'>KPI: VARIATION ACCIDENTS RATE</h1>",
            unsafe_allow_html=True)
st.markdown("***")
st.markdown("This KPI describes the variation in the number of accidents that occurred in one year compared to the previous one.")
st.markdown("<h3 style='text-align: center; color: black;'>Rate=(N°Accidents (N)year-N°Accicents(N-1)Year) /N°Accicents(N-1)Year</h3>",
            unsafe_allow_html=True)
...
if bu1:
    ...
    All_fat["Rate goal"] = All_fat["All Fatalities"]
    ...
    All_fat["Rate goal"] = All_fat["Rate"]
    for i in range(All_fat.shape[0]-1):
        All_fat["Rate goal"].iloc[i+1] = All_fat["Rate"].iloc[i]-10
    st.dataframe(All_fat)

    fig1 = px.line(All_fat, x="Year", y=[
...
if st.session_state['btn']:
    ...
    with a2:
        ...
        All_fat = All_fat[All_fat["Country"] == pais].reset_index(drop=True)
        All_fat["Rate"] = All_fat["All Fatalities"]
        All_fat["Rate goal"] = All_fat["All Fatalities"]
        for i in range(All_fat.shape[0]-1):
            All_fat["Rate"].iloc[i+1] = 100*(All_fat["All Fatalities"].iloc[i+1] -
        ...
        st.dataframe(All_fat[["Year", "Country", "All Fatalities", "Rate", "Rate goal"]][(All_fat["Year"] >= values[0])
                     & (All_fat["Year"] <= values[1])])

    fig1 = px.line(All_fat[(All_fat["Year"] >= values[0]) & (All_fat["Year"] <= values[1])], x="Year", y=[
        "Rate", "Rate goal"], title="Mortality Rate").update_layout(
        xaxis_title="Year", yaxis_title="Rate"
    )
    st.write(fig1)
